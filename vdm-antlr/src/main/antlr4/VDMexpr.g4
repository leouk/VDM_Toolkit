...
//------------------------
      PAREN_L expression PAREN_R                            #BracketedExpr    // (expr)
//------------------------
...
//------------------------
    ...
    | symbolic_literal                                      #SymbolicLitExpr  // literal
    ;
...
//         BRACE_L 
//         type_list 
//         BRACE_R                                             #FunctionTypeInstExpr             // expr[TYPES]
// //------------------------
// // C.0 The family of suffix expressions binders
// //------------------------
//     | SLK_let local_definition_list 
//       SLK_in expression                                     #LetExpr                           // let LOCALS in expr
//     | SLK_let multiple_bind (SLK_best stexpr=expression)? 
//       SLK_in inexpr=expression                              #LetBestExpr                       // let BIND [be st expr] in expr
//     | SLK_def equals_definition_list SEP_scolon?
//       SLK_in expression                                     #DefExpr                           // def LOCALS ; expr
//------------------------
...
// //------------------------
     |<assoc=right> iter=expression O_EXP power=expression   #IterateExpr                       // expr ** expr
//     |<assoc=right> lhs=expression SLK_comp rhs=expression   #MapCompositionExpr                // expr comp expr
...
//------------------------
    |              lhs=expression (O_GEQ | O_LEQ | O_GT | O_LT)      rhs=expression      #RelationalGreaterThanEqualExpr   //61 relations(1)   
    ...
    |<assoc=right> lhs=expression (O_EQUAL | O_NEQ)       rhs=expression      #RelationalNotEqualExpr           //65 relations(5)
    // |<assoc=right> lhs=expression (O_EQUAL     rhs=expression)      #RelationalEqualExpr              //66 relations(6)
    |              lhs=expression (SLK_ninset | SLK_inset | SLK_psubset | SLK_subset)  rhs=expression      #SetNotMemberExpr                 //67 relations(7)
