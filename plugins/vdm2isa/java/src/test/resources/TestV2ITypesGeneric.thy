(* VDM to Isabelle Translation @2021-12-23T09:27:27.485Z
...
where
	"post_injective inv_T ss  RESULT \<equiv> 
		...
		((inv_VDMSeq'  \<comment>\<open>VDM generic type `T` will become an Isabelle type parameter. Specific instantiations require invariant checking function\<close> inv_T ss)  \<and>  (inv_bool RESULT))"

definition
	injective :: "'T VDMSeq \<Rightarrow> bool"
where
	"injective ss \<equiv> 
...
\<comment>\<open>VDM source: f[S, T]: (seq of (@S) -> seq of (@T))
	f(s) ==
s\<close>
\<comment>\<open>in 'TestV2ITypesGeneric' (/Users/nljsf/Local/reps/git/VDM_Toolkit/plugins/vdm2isa/java/src/test/resources/TestV2ITypesGeneric.vdmsl) at line 11:5\<close>
...
where
	"pre_f inv_S s \<equiv> 
		...
		((inv_VDMSeq'  \<comment>\<open>VDM generic type `S` will become an Isabelle type parameter. Specific instantiations require invariant checking function\<close> inv_S s))"
...
where
	"post_f inv_S s inv_T RESULT \<equiv> 
		...
		((inv_VDMSeq'  \<comment>\<open>VDM generic type `S` will become an Isabelle type parameter. Specific instantiations require invariant checking function\<close> inv_S s)  \<and>  (inv_VDMSeq'  \<comment>\<open>VDM generic type `T` will become an Isabelle type parameter. Specific instantiations require invariant checking function\<close> inv_T RESULT))"
...
where
	...
	if inv_VDMSeq' inv_S s \<and> inv_VDMSeq' inv_T [] then
    []
  else
    undefined"
