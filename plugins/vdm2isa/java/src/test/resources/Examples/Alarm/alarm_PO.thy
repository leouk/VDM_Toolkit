(* VDM to Isabelle Translation @2022-02-27T08:15:41.636641Z
...
theorem Expert_INV_SATISFIABILITY_PO2:
	...
	by (metis Expert.select_convs(2) insert_not_empty)
...
theorem Plant_INV_SATISFIABILITY_PO7:
	...
	by (metis Plant.select_convs(1) empty_iff l_map_with_range_not_dom_empty ran_empty)
...
theorem ExpertToPage_TOTAL_PO10:
	...
	"((\<forall> (a :: Alarm)   (peri :: Period)   (plant :: Plant)  . ((( (((inv_VDMSeq' (inv_VDMChar) (alarmtext\<^sub>A\<^sub>l\<^sub>a\<^sub>r\<^sub>m a))) \<and> 
		 ...
		 ((inv_VDMSet' inv_Alarm  (alarms\<^sub>P\<^sub>l\<^sub>a\<^sub>n\<^sub>t plant))) ))) \<longrightarrow> isTest ((pre_ExpertToPage a   peri   plant)) (inv_bool))))"
	by simp
...
theorem ExpertToPage_MAP_APPLY_PO11:
	...
	"((\<forall> (a :: Alarm)   (peri :: Period)   (plant :: Plant)  . (((
		...
		((((inv_Map ((inv_VDMToken' (inv_VDMSeq' (inv_VDMChar)))) (inv_VDMSet' inv_Expert ) (schedule\<^sub>P\<^sub>l\<^sub>a\<^sub>n\<^sub>t plant)))) \<and> 
		 ((inv_VDMSet' inv_Alarm  (alarms\<^sub>P\<^sub>l\<^sub>a\<^sub>n\<^sub>t plant)))
		))) \<longrightarrow> (((peri \<in> (dom (schedule\<^sub>P\<^sub>l\<^sub>a\<^sub>n\<^sub>t plant))) \<and> (a \<in> (alarms\<^sub>P\<^sub>l\<^sub>a\<^sub>n\<^sub>t plant))) \<longrightarrow> (\<forall> (r :: Expert)  . (((
		((((inv_VDMToken' (inv_VDMSeq' (inv_VDMChar)) (expertid\<^sub>E\<^sub>x\<^sub>p\<^sub>e\<^sub>r\<^sub>t r)))) \<and> 
		 ((inv_VDMSet' ((inv_Qualification )) (quali\<^sub>E\<^sub>x\<^sub>p\<^sub>e\<^sub>r\<^sub>t r)))
		))) \<longrightarrow> (peri \<in> (dom (schedule\<^sub>P\<^sub>l\<^sub>a\<^sub>n\<^sub>t plant)))))))))"
	by presburger
...
theorem ExpertToPage_FUNC_SATISFIABILITY_PO12:
	...
	"((\<forall> (a :: Alarm)   (peri :: Period)   (plant :: Plant)  . ((( (((inv_VDMSeq' (inv_VDMChar) (alarmtext\<^sub>A\<^sub>l\<^sub>a\<^sub>r\<^sub>m a))) \<and> 
		 ((((inv_True (quali\<^sub>A\<^sub>l\<^sub>a\<^sub>r\<^sub>m a))))) )))  \<and>  ((((inv_VDMToken' (inv_VDMSeq' (inv_VDMChar)) peri))))  \<and>  (( ((((inv_Map ((inv_VDMToken' (inv_VDMSeq' (inv_VDMChar)))) (inv_VDMSet' inv_Expert ) (schedule\<^sub>P\<^sub>l\<^sub>a\<^sub>n\<^sub>t plant)))) \<and> 
		 ((inv_VDMSet' inv_Alarm  (alarms\<^sub>P\<^sub>l\<^sub>a\<^sub>n\<^sub>t plant))) ))) \<longrightarrow> ((pre_ExpertToPage a   peri   plant) \<longrightarrow> (\<exists> (r :: Expert)  . ((( ((((inv_VDMToken' (inv_VDMSeq' (inv_VDMChar)) (expertid\<^sub>E\<^sub>x\<^sub>p\<^sub>e\<^sub>r\<^sub>t r)))) \<and> 
		 ((inv_VDMSet' ((inv_Qualification )) (quali\<^sub>E\<^sub>x\<^sub>p\<^sub>e\<^sub>r\<^sub>t r))) ))) \<longrightarrow> (post_ExpertToPage a   peri   plant   r)))))))"
	
	by (metis QualificationOK_def empty_iff inv_ExpertId_def inv_Expert_def inv_Plant_def post_ExpertToPage_def pre_ExpertToPage_def)
...
theorem ChangeExpert_SUB_TYPE_PO14:
	...
	apply (simp, safe)
  unfolding inv_Schedule_defs Let_def apply (simp, safe, simp_all)
	oops
...
theorem plant1_SUB_TYPE_PO25:
	...
	unfolding inv_Schedule_defs by simp
...
theorem plant1_SUB_TYPE_PO26:
	...
	unfolding inv_Schedule_defs inv_Plant_defs QualificationOK_def by simp
