(* VDM to Isabelle Translation @2022-02-27T10:23:41.139Z
   Copyright 2021, Leo Freitas, leo.freitas@newcastle.ac.uk

in 'gateway' (/Users/nljsf/Local/reps/git/VDM_Toolkit/plugins/vdm2isa/java/src/test/resources/Examples/Gateway/gateway.vdmsl) at line 11:7
...
theorem Message_TOTAL_PO3:
	\<comment>\<open>Implicitly defined type invariant checks for quantified type binds\<close> 
	"((\<forall> (m :: String)  . (((((inv_VDMSeq' (inv_VDMChar) m)))) \<longrightarrow> isTest ((inv_Message m)) (inv_bool))))"
	by simp
...
theorem Message_INV_SATISFIABILITY_PO4:
	\<comment>\<open>Implicitly defined type invariant checks for quantified type binds\<close> 
	"((\<exists> (m :: String)  . (((((inv_VDMSeq' (inv_VDMChar) m)))) \<longrightarrow> ((len m) \<le> (100::VDMNat1)))))"
	by (metis l_len_empty zero_le_numeral)
...
\<comment>\<open>in 'gateway' (/Users/nljsf/Local/reps/git/VDM_Toolkit/plugins/vdm2isa/java/src/test/resources/Examples/Gateway/gateway.vdmsl) at line 58:12\<close>
theorem Gateway_measure_Gateway_TOTAL_PO5:
	\<comment>\<open>Implicitly defined type invariant checks for quantified type binds\<close> 
	"((\<forall> (ms :: Message VDMSeq)   (cat :: Category)  . ((((inv_VDMSeq' (inv_String) ms)))  \<and>  ((((inv_VDMSet' ((inv_VDMSeq' (inv_VDMChar))) cat)))) \<longrightarrow> isTest ((measure_Gateway ms   cat)) (inv_VDMNat))))"
...
theorem Gateway_rest_p_RECURSIVE_PO6:
	\<comment>\<open>Implicitly defined type invariant checks for quantified type binds\<close> 
	"((\<forall> (ms :: Message VDMSeq)   (cat :: Category)  . ((((inv_VDMSeq' (inv_String) ms)))  \<and>  ((((inv_VDMSet' ((inv_VDMSeq' (inv_VDMChar))) cat)))) \<longrightarrow> ((\<not> (ms = [])) \<longrightarrow> ((measure_Gateway ms   cat) > (measure_Gateway (tl ms)   cat))))))"
...
\<comment>\<open>VDM source: Gateway; rest_p = ?\<close>
\<comment>\<open>in 'gateway' (/Users/nljsf/Local/reps/git/VDM_Toolkit/plugins/vdm2isa/java/src/test/resources/Examples/Gateway/gateway.vdmsl) at line 54:34\<close>
theorem Gateway_rest_p_NON_EMPTY_SEQ_PO7:
	\<comment>\<open>Implicitly defined type invariant checks for quantified type binds\<close> 
	"((\<forall> (ms :: Message VDMSeq)   (cat :: Category)  . ((((inv_VDMSeq' (inv_String) ms)))  \<and>  ((((inv_VDMSet' ((inv_VDMSeq' (inv_VDMChar))) cat)))) \<longrightarrow> ((\<not> (ms = [])) \<longrightarrow> (ms \<noteq> [])))))"
...
\<comment>\<open>in 'gateway' (/Users/nljsf/Local/reps/git/VDM_Toolkit/plugins/vdm2isa/java/src/test/resources/Examples/Gateway/gateway.vdmsl) at line 56:30\<close>
theorem Gateway_NON_EMPTY_SEQ_PO8:
	\<comment>\<open>Implicitly defined type invariant checks for quantified type binds\<close> 
	"((\<forall> (ms :: Message VDMSeq)   (cat :: Category)  . ((((inv_VDMSeq' (inv_String) ms)))  \<and>  ((((inv_VDMSet' ((inv_VDMSeq' (inv_VDMChar))) cat)))) \<longrightarrow> ((\<not> (ms = [])) \<longrightarrow> (
		let 
...
lemma Gateway_NON_EMPTY_SEQ_PO8_gen:
	\<comment>\<open>Implicitly defined type invariant checks for quantified type binds\<close> 
	"(( ((((inv_VDMSeq' (inv_String) ms)))  \<and>  ((((inv_VDMSet' ((inv_VDMSeq' (inv_VDMChar))) cat)))) \<longrightarrow> ((\<not> (ms = [])) \<longrightarrow> (
		let 
(rest_p::Ports) = (Gateway (tl ms)   cat)
		in
			(if (inv_Ports rest_p) then
