...
where
	\<open>PO_41_Map_TOTAL  \<equiv> (\<forall> (m :: (S \<rightharpoonup> Rec))  . ((((inv_Map inv_S inv_Rec  m))) \<longrightarrow> isTest ((inv_Map m)) (inv_bool)))\<close>
...
where
	\<open>PO_42_Map_INV_SATISFIABILITY  \<equiv> (\<exists> (m :: (S \<rightharpoonup> Rec))  . ((((inv_Map inv_S inv_Rec  m))) \<longrightarrow> ((dom m) = {})))\<close>
