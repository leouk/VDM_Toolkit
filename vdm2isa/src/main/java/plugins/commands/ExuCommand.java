...
public class ExuCommand extends IsabelleCommand
{
	...
	private static final String USAGE = "exu - it analyses loaded VDM modules for Isabelle/HOL  (v. "
			+ IsaProperties.general_isa_version + ") translation";
	...
	protected boolean setup()
	{
		...
		// some VDM warnings have to be raised as Exu warnings or errors depending on
		// strictness
	...
	protected void checkSpecificationCallsConsistency(TCModuleList tclist, VDMSpecificationKind kind,
			TCExplicitFunctionDefinition spec)
	{
		if (spec != null)
		{
			...
			for (TCNameToken n : found)
			{
				...
				if (!isSpecificationName(n))
				{
					...
					if (!found.contains(pren))
					{
						stack.clear();
						stack.push(pren);
						TCDefinitionList predef = findDefinitions(tclist, stack);
						// there isn't a pre defined? Okay what about the type invariants?
						if (predef == null || predef.isEmpty())
						{
							warning(IsaWarningMessage.VDMSL_EXU_MISSING_INVCALL_3P, n.getLocation(),
									spec.name.toString(),
									kind.equals(VDMSpecificationKind.NONE) ? "body" : kind.name().toLowerCase(), n);
						}
						// there is a pre defined, then why haven't you called it?
						else
						{
							warning(IsaWarningMessage.VDMSL_EXU_MISSING_SPECCALL_3P, n.getLocation(),
									spec.name.toString(),
									kind.equals(VDMSpecificationKind.NONE) ? "body" : kind.name().toLowerCase(), n);
						}
					}
	...
	// {
	// report(IsaErrorMessage.VDMSL_EXU_IMPLICIT_FUNCTION_BODY_1P,
	// def.name.getLocation(), def.name.toString());
	// }
	// }
	...
	protected void checkSpecificationDependencies(TCDefinition d, TCModuleList tclist)
	{
		if (d instanceof TCTypeDefinition)
		{
			...
			// if (tdef.eqdef == null && tdef.orddef != null || tdef.eqdef != null &&
			// tdef.orddef == null)
			// {
			// report(IsaErrorMessage.ISA_INVALID_INVTYP_2P, getName(), "must define both eq
			// and ord specificaition");
			// }
	...
	{
		return super.options() + String.format(" linv=%1$s rtc=%2$s gs=%3$s", IsaProperties.exu_linient_inv_check,
				IsaProperties.exu_retypecheck, IsaProperties.exu_print_sorted_graph);
	}
	...
	public TCDefinitionList findDefinitions(TCModuleList modules, Stack<TCNameToken> stack)
	{
		TCDefinitionList list = new TCDefinitionList();

		for (TCNameToken name : stack)
		{
			list.add(findDefinition(modules, name));
		}

		return list.contains(null) ? null : list; // Usually local func definitions
	}

	private TCDefinition findDefinition(TCModuleList modules, TCNameToken sought)
	{
